...
for c in container:
    #if cancel_btn = driver.find_element_by_css_selector("button._closeAllTimeAlert.expires.layer_time_text_button")
    #    cancel_btn.click()
...
# 티몬 연극
timon_play = pd.read_csv('./timon_play.csv',encoding='UTF-8')
timon_play_title=timon_play['제목']
timon_play_title=timon_play_title.tolist()

timon_play_place1 = timon_play['장소']
timon_play_place1 = timon_play_place1.tolist()
timon_play_place = timon_play['장소']
timon_play_place = timon_play_place.tolist()

timon_play_comp1 = timon_play['주최/기획']
timon_play_comp1 = timon_play_comp1.tolist()
timon_play_comp = timon_play['주최/기획']
timon_play_comp = timon_play_comp.tolist()

for i in range(len(timon_play_title)):
    timon_play_title[i] = (timon_play_title[i]).replace('뮤지컬', '').replace('연극', '').replace('대학로', '').replace('부산', '').    replace('홍대', '').replace('[', '').replace(']', '').replace('새해선물', '').replace('오늘의컬쳐', '').replace('〈'    , '').replace('〉', '').replace(' ', '').replace('  ', '').replace('   ', '');
    timon_play_title[i] = ' '.join(timon_play_title[i])
    timon_play_title[i] = 'a'.join(str(x) for x in timon_play_title[i])
    timon_play_title[i] = 'a' + timon_play_title[i] + 'a'

    timon_play_place[i] = str(timon_play_place[i]).replace(" ", "").replace("씨어터", "").replace("시어터", "")
    timon_play_place[i] = timon_play_place[i].upper()

    timon_play_comp[i] = str(timon_play_comp[i]).replace(" ", "")


timon_play_list = []
timon_play_df = pd.DataFrame(timon_play_list, columns = ['제목', '티몬가격', '티몬링크'])
temp = 0

for i in range(len(timon_play_title)):
    max = 0
    for j in range(len(interpark_title)):
        compare_list = [timon_play_title[i], interpark_title[j]]
        ...
        if timon_play_place[i] == interpark_place[j]:
            distances2 = 2
        elif timon_play_place[i] in interpark_place[j]:
            distances2 = 1
        elif interpark_place[j] in timon_play_place[i]:
            distances2 = 1
        else: distances2 = 0

        if timon_play_comp[i] == interpark_comp[j]:
            distances3 = 1
        else:
            distances3 = 0
    ...
    if max > 1.25:
        ...
        price = timon_play['티몬가격'][i]
        link = timon_play['티몬링크'][i]
        timon_play_df.loc[temp] = [name, price, link]
        temp = temp + 1

timon_play_df.to_csv('timon_play_final.csv', encoding="UTF-8")
